...
function recalculate() {
  ...
  discountRowsGlobal.forEach(row => {
    ...
    tr.innerHTML = `
      <td class="px-2 py-1.5 text-slate-400">${row.est}</td>
      <td class="px-2 py-1.5">${row.concepto}</td>
      <td class="px-2 py-1.5 text-right font-mono text-slate-500">RD$ ${fmt(row.base)}</td>
      <td class="px-2 py-1.5 text-right font-mono ${colorClass}">${row.pct.toFixed(2)}%</td>
      <td class="px-2 py-1.5 text-right font-mono ${colorClass}">${sign} RD$ ${fmt(row.monto)}</td>
    `;
  ...
  $("totalDescuentos").className = `font-black text-[11px] ${totalDiff < 0 ? "text-red-500" : "text-emerald-600"}`;
  ...
  extraPrograms.forEach(ep => {
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td class="px-2 py-1.5 text-slate-400">${stNum}</td>
      <td class="px-2 py-1.5">${name}</td>
      <td class="px-2 py-1.5">${distType} cuota(s)</td>
      <td class="px-2 py-1.5 text-right font-mono">RD$ ${fmt(cost)}</td>
    `;
  ...
  for (let i = 0; i < totalRows; i++) {
    ...
    const tr = document.createElement("tr");
    tr.innerHTML = `
      <td class="px-2 py-1.5">${concepto}</td>
      <td class="px-2 py-1.5 text-right font-mono">RD$ ${fmt(colegAmt)}</td>
      <td class="px-2 py-1.5 text-right font-mono">${extraAmt > 0 ? "RD$ " + fmt(extraAmt) : "—"}</td>
      <td class="px-2 py-1.5 text-right font-mono font-bold">RD$ ${fmt(total)}</td>
    `;
